// tests/rna/pipelines/ingestion/stages/intake/intake.stage.test.ts
...
test("does not pre-reject on semantic grounds (intake never judges meaning)", () => {
  ...
  assert.equal(s.proposalId, env.ids.proposalId);
  assert.ok(typeof s.intakeTimestampMs === "number");
});
